...
import { Task, KanbanColumn } from '@turbo-with-tailwind-v4/database/types';
...
export const KanbanPositionTester: React.FC = () => {
  const [tasks, setTasks] = useState<Omit<Task, 'created_at' | 'updated_at'>[]>([
    { id: 'task1', column_id: 'todo', title: 'Design Homepage', position: 1000, status: 'todo' },
    { id: 'task2', column_id: 'todo', title: 'Create API', position: 2000, status: 'todo' },
    { id: 'task3', column_id: 'todo', title: 'Write Tests', position: 3000, status: 'todo' },
    { id: 'task4', column_id: 'progress', title: 'Setup Database', position: 1000, status: 'inprogress' },
    { id: 'task5', column_id: 'progress', title: 'Deploy App', position: 2000, status: 'inprogress' },
    { id: 'task6', column_id: 'done', title: 'Project Setup', position: 1000, status: 'done' },
  ]);
  ...
  // Replace columns array with KanbanColumn[]
  ...
  const moveTask = () => {
    try {
      ...
      if (selectedColumn === 'progress') newStatus = 'inprogress';
      if (selectedColumn === 'done') newStatus = 'done';
      const updatedTasks = result.updatedTasks.map(t =>
        t.id === selectedTask ? { ...t, status: newStatus, column_id: selectedColumn } : t
      );
  ...
  const addTask = () => {
    ...
    if (columnId === 'progress') status = 'inprogress';
  ...
  const reset = () => {
    ...
    addTestResult('🔄 Reset to initial state', 'info');
  };
                ...
                <div className="space-y-2">
                  {columnTasks.map((task, index) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-400">Index: {index}</div>
                    </div>
                  ))}
