...
// input takes a callback function to add the task
const TasksInput = ({ addTask }: { addTask: (title: string) => void }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex w-full items-center space-x-2 p-2 border-b">
            ...
            />
            <Button className="bg-blue-500 hover:bg-blue-600 text-white rounded-full p-4 text-lg focus:ring-0" type="submit" disabled={!title.trim()}>
                 <Plus />
            </Button>
        </form>
    )
